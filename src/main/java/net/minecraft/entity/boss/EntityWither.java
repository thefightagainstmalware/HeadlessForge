...
public class EntityWither extends EntityMob implements IBossDisplayData, IRangedAttackMob
{
    ...
    public void onLivingUpdate()
    {
        ...
        if (!this.field_70170_p.isRemote && this.getWatchedTargetId(0) > 0)
        {
            Entity entity = this.field_70170_p.getEntityByID(this.getWatchedTargetId(0));

            if (entity != null)
            {
                if (this.field_70163_u < entity.field_70163_u || !this.isArmored() && this.field_70163_u < entity.field_70163_u + 5.0D)
                {
        ...
        {
            this.field_70177_z = (float)MathHelper.func_181159_b(this.motionZ, this.motionX) * (180F / (float)Math.PI) - 90.0F;
        }
        ...
        for (int j = 0; j < 2; ++j)
        {
            ...
            if (k > 0)
            {
                entity1 = this.field_70170_p.getEntityByID(k);
            }
            ...
            if (entity1 != null)
            {
                ...
                double d7 = entity1.field_70163_u + (double)entity1.getEyeHeight() - d12;
        ...
        for (int l = 0; l < 3; ++l)
        {
            ...
            this.field_70170_p.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.30000001192092896D, 0.0D, 0.0D, 0.0D, new int[0]);

            if (flag && this.field_70170_p.rand.nextInt(4) == 0)
            {
                this.field_70170_p.spawnParticle(EnumParticleTypes.SPELL_MOB, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.30000001192092896D, 0.699999988079071D, 0.699999988079071D, 0.5D, new int[0]);
            }
        }
        ...
        if (this.getInvulTime() > 0)
        {
            ...
            {
                this.field_70170_p.spawnParticle(EnumParticleTypes.SPELL_MOB, this.field_70165_t + this.rand.nextGaussian() * 1.0D, this.field_70163_u + (double)(this.rand.nextFloat() * 3.3F), this.field_70161_v + this.rand.nextGaussian() * 1.0D, 0.699999988079071D, 0.699999988079071D, 0.8999999761581421D, new int[0]);
            }
        }
    }
    ...
    protected void updateAITasks()
    {
        if (this.getInvulTime() > 0)
        {
            ...
            if (j1 <= 0)
            {
                this.field_70170_p.newExplosion(this, this.field_70165_t, this.field_70163_u + (double)this.getEyeHeight(), this.field_70161_v, 7.0F, false, this.field_70170_p.func_82736_K().func_82766_b("mobGriefing"));
                this.field_70170_p.playBroadcastSound(1013, new BlockPos(this), 0);
            }
        ...
        else
        {
            ...
            for (int i = 1; i < 3; ++i)
            {
                if (this.ticksExisted >= this.field_82223_h[i - 1])
                {
                    ...
                    if (this.field_70170_p.getDifficulty() == EnumDifficulty.NORMAL || this.field_70170_p.getDifficulty() == EnumDifficulty.HARD)
                    {
                        int j3 = i - 1;
                        ...
                        if (k3 > 15)
                        {
                            float f = 10.0F;
                            float f1 = 5.0F;
                            double d0 = MathHelper.func_82716_a(this.rand, this.field_70165_t - (double)f, this.field_70165_t + (double)f);
                            double d1 = MathHelper.func_82716_a(this.rand, this.field_70163_u - (double)f1, this.field_70163_u + (double)f1);
                            double d2 = MathHelper.func_82716_a(this.rand, this.field_70161_v - (double)f, this.field_70161_v + (double)f);
                    ...
                    if (k1 > 0)
                    {
                        Entity entity = this.field_70170_p.getEntityByID(k1);

                        if (entity != null && entity.isEntityAlive() && this.getDistanceSqToEntity(entity) <= 900.0D && this.canEntityBeSeen(entity))
                        {
                            if (entity instanceof EntityPlayer && ((EntityPlayer)entity).field_71075_bZ.disableDamage)
                            {
                    ...
                    {
                        List<EntityLivingBase> list = this.field_70170_p.<EntityLivingBase>getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().expand(20.0D, 8.0D, 20.0D), Predicates.<EntityLivingBase> and (attackEntitySelector, EntitySelectors.NOT_SPECTATING));
                        ...
                        for (int j2 = 0; j2 < 10 && !list.isEmpty(); ++j2)
                        {
                            ...
                            if (entitylivingbase != this && entitylivingbase.isEntityAlive() && this.canEntityBeSeen(entitylivingbase))
                            {
                                ...
                                {
                                    if (!((EntityPlayer)entitylivingbase).field_71075_bZ.disableDamage)
                                    {
            ...
            if (this.blockBreakCounter > 0)
            {
                ...
                if (this.blockBreakCounter == 0 && this.field_70170_p.func_82736_K().func_82766_b("mobGriefing"))
                {
                    ...
                    if (flag)
                    {
                        this.field_70170_p.playAuxSFXAtEntity((EntityPlayer)null, 1012, new BlockPos(this), 0);
                    }
                }
            }
    ...
    {
        return p_181033_0_ != Blocks.field_150357_h && p_181033_0_ != Blocks.end_portal && p_181033_0_ != Blocks.end_portal_frame && p_181033_0_ != Blocks.command_block && p_181033_0_ != Blocks.field_180401_cv;
    }
    ...
    {
        this.launchWitherSkullToCoords(p_82216_1_, p_82216_2_.field_70165_t, p_82216_2_.field_70163_u + (double)p_82216_2_.getEyeHeight() * 0.5D, p_82216_2_.field_70161_v, p_82216_1_ == 0 && this.rand.nextFloat() < 0.001F);
    }
    ...
    {
        this.field_70170_p.playAuxSFXAtEntity((EntityPlayer)null, 1014, new BlockPos(this), 0);
    ...
    {
        EntityItem entityitem = this.dropItem(Items.field_151156_bN, 1);
        ...
        {
            for (EntityPlayer entityplayer : this.field_70170_p.getEntitiesWithinAABB(EntityPlayer.class, this.getEntityBoundingBox().expand(50.0D, 100.0D, 50.0D)))
            {
                entityplayer.func_71029_a(AchievementList.killWither);
            }
        }
    }
