    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ)
    {
        ...
        else
        {
            ...
            if ((side != EnumFacing.UP || block != this.block) && !block.isReplaceable(worldIn, pos))
            {
                ...
                iblockstate = worldIn.func_180495_p(blockpos);
            ...
            if (block == this.block)
            {
                ...
                if (i <= 7)
                {
                    ...
                    if (axisalignedbb != null && worldIn.checkNoEntityCollision(axisalignedbb) && worldIn.func_180501_a(blockpos, iblockstate1, 2))
                    {
                        worldIn.playSoundEffect((double)((float)blockpos.func_177958_n() + 0.5F), (double)((float)blockpos.func_177956_o() + 0.5F), (double)((float)blockpos.func_177952_p() + 0.5F), this.block.stepSound.getPlaceSound(), (this.block.stepSound.getVolume() + 1.0F) / 2.0F, this.block.stepSound.getFrequency() * 0.8F);
                        --stack.field_77994_a;
                        return true;
                    }
                }
