...
public class ItemPotion extends Item
{
    ...
    public List<PotionEffect> getEffects(ItemStack stack)
    {
        if (stack.func_77942_o() && stack.func_77978_p().func_150297_b("CustomPotionEffects", 9))
        {
            ...
            NBTTagList nbttaglist = stack.func_77978_p().getTagList("CustomPotionEffects", 10);
    ...
    public ItemStack onItemUseFinish(ItemStack stack, World worldIn, EntityPlayer playerIn)
    {
        ...
        if (!playerIn.field_71075_bZ.field_75098_d)
        {
            ...
            playerIn.field_71071_by.func_70441_a(new ItemStack(Items.glass_bottle));
        }
    ...
    public ItemStack onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer playerIn)
    {
        if (isSplash(itemStackIn.getMetadata()))
        {
            ...
            worldIn.func_72956_a(playerIn, "random.bow", 0.5F, 0.4F / (itemRand.nextFloat() * 0.4F + 0.8F));

            if (!worldIn.isRemote)
            {
                worldIn.func_72838_d(new EntityPotion(worldIn, playerIn, itemStackIn));
            }
    ...
    public boolean isEffectInstant(int meta)
    {
        ...
        if (list != null && !list.isEmpty())
        {
            for (PotionEffect potioneffect : list)
            {
                if (Potion.field_76425_a[potioneffect.getPotionID()].func_76403_b())
                {
                    return true;
                }
            }
    ...
    public void addInformation(ItemStack stack, EntityPlayer playerIn, List<String> tooltip, boolean advanced)
    {
        if (stack.getMetadata() != 0)
        {
            ...
            if (list != null && !list.isEmpty())
            {
                for (PotionEffect potioneffect : list)
                {
                    String s1 = StatCollector.translateToLocal(potioneffect.func_76453_d()).trim();
                    Potion potion = Potion.field_76425_a[potioneffect.getPotionID()];
