...
public final class SpawnerAnimals
{
                    ...
                    for (int i1 = -l; i1 <= l; ++i1)
                    {
                        for (int j1 = -l; j1 <= l; ++j1)
                        {
                            ...
                            if (!this.eligibleChunksForSpawning.contains(chunkcoordintpair))
                            {
                                ...
                                if (!flag && p_77192_1_.func_175723_af().contains(chunkcoordintpair))
                                {
                                    ...
                                    for (int l3 = 0; l3 < 4; ++l3)
                                    {
                                        ...
                                        if (!p_77192_1_.isAnyPlayerWithinRangeAt((double)f, (double)i3, (double)f1, 24.0D) && blockpos2.distanceSq((double)f, (double)i3, (double)f1) >= 576.0D)
                                        {
                                            ...
                                            if (p_77192_1_.canCreatureTypeSpawnHere(enumcreaturetype, biomegenbase$spawnlistentry, blockpos1) && canCreatureTypeSpawnAtLocation(EntitySpawnPlacementRegistry.getPlacementForEntity(biomegenbase$spawnlistentry.entityClass), p_77192_1_, blockpos1))
                                            {
                                                ...
                                                entityliving.func_70012_b((double)f, (double)i3, (double)f1, p_77192_1_.rand.nextFloat() * 360.0F, 0.0F);
    ...
    {
        if (!worldIn.func_175723_af().contains(pos))
        {
            return false;
        }
        else
        {
            ...
            {
                return block.func_149688_o().func_76224_d() && worldIn.func_180495_p(pos.func_177977_b()).func_177230_c().func_149688_o().func_76224_d() && !worldIn.func_180495_p(pos.up()).func_177230_c().isNormalCube();
            }
            else
            {
                ...
                {
                    Block block1 = worldIn.func_180495_p(blockpos).func_177230_c();
                    boolean flag = block1 != Blocks.field_150357_h && block1 != Blocks.field_180401_cv;
                    return flag && !block.isNormalCube() && !block.func_149688_o().func_76224_d() && !worldIn.func_180495_p(pos.up()).func_177230_c().isNormalCube();
                }
            }
        }
    }
                ...
                for (int j1 = 0; j1 < i; ++j1)
                {
                    ...
                    for (int k1 = 0; !flag && k1 < 4; ++k1)
                    {
                        ...
                        if (canCreatureTypeSpawnAtLocation(EntityLiving.SpawnPlacementType.ON_GROUND, worldIn, blockpos))
                        {
                            ...
                            ientitylivingdata = entityliving.func_180482_a(worldIn.func_175649_E(new BlockPos(entityliving)), ientitylivingdata);
                            flag = true;
                        }
