...
public class WorldGenMegaJungle extends WorldGenHugeTrees
{
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        else
        {
            ...
            for (int j = position.func_177956_o() + i - 2 - rand.nextInt(4); j > position.func_177956_o() + i / 2; j -= 2 + rand.nextInt(4))
            {
                ...
                int k = position.func_177958_n() + (int)(0.5F + MathHelper.cos(f) * 4.0F);
                int l = position.func_177952_p() + (int)(0.5F + MathHelper.sin(f) * 4.0F);

                for (int i1 = 0; i1 < 5; ++i1)
                {
                    k = position.func_177958_n() + (int)(1.5F + MathHelper.cos(f) * (float)i1);
                    l = position.func_177952_p() + (int)(1.5F + MathHelper.sin(f) * (float)i1);
    ...
    private void func_181632_a(World p_181632_1_, Random p_181632_2_, BlockPos p_181632_3_, PropertyBool p_181632_4_)
    {
        ...
        {
            this.setBlockAndNotifyAdequately(p_181632_1_, p_181632_3_, Blocks.vine.func_176223_P().withProperty(p_181632_4_, Boolean.valueOf(true)));
        }
    }
