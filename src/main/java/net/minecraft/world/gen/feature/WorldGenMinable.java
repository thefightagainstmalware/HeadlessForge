...
public class WorldGenMinable extends WorldGenerator
{
    ...
    {
        this(state, blockCount, BlockHelper.forBlock(Blocks.field_150348_b));
    }
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        double d0 = (double)((float)(position.func_177958_n() + 8) + MathHelper.sin(f) * (float)this.numberOfBlocks / 8.0F);
        double d1 = (double)((float)(position.func_177958_n() + 8) - MathHelper.sin(f) * (float)this.numberOfBlocks / 8.0F);
        double d2 = (double)((float)(position.func_177952_p() + 8) + MathHelper.cos(f) * (float)this.numberOfBlocks / 8.0F);
        double d3 = (double)((float)(position.func_177952_p() + 8) - MathHelper.cos(f) * (float)this.numberOfBlocks / 8.0F);
                        ...
                        if (d12 * d12 + d13 * d13 < 1.0D)
                        {
                            for (int j2 = l; j2 <= k1; ++j2)
                            {
                                ...
                                if (d12 * d12 + d13 * d13 + d14 * d14 < 1.0D)
                                {
                                    ...
                                    if (this.predicate.apply(worldIn.func_180495_p(blockpos)))
                                    {
