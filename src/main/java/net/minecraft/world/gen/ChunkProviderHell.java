...
public class ChunkProviderHell implements IChunkProvider
{
    ...
    private final WorldGenerator field_177467_w = new WorldGenMinable(Blocks.field_150449_bY.func_176223_P(), 14, BlockHelper.forBlock(Blocks.netherrack));
                    ...
                    for (int i2 = 0; i2 < 8; ++i2)
                    {
                        ...
                        for (int j2 = 0; j2 < 4; ++j2)
                        {
                            ...
                            for (int k2 = 0; k2 < 4; ++k2)
                            {
                                ...
                                if (l1 * 8 + i2 < j)
                                {
                                    iblockstate = Blocks.lava.func_176223_P();
                                }

                                if (d15 > 0.0D)
                                {
                                    iblockstate = Blocks.netherrack.func_176223_P();
                                }
    ...
    public void func_180516_b(int p_180516_1_, int p_180516_2_, ChunkPrimer p_180516_3_)
    {
        ...
        for (int j = 0; j < 16; ++j)
        {
            for (int k = 0; k < 16; ++k)
            {
                ...
                int i1 = -1;
                IBlockState iblockstate = Blocks.netherrack.func_176223_P();
                IBlockState iblockstate1 = Blocks.netherrack.func_176223_P();
                ...
                for (int j1 = 127; j1 >= 0; --j1)
                {
                    if (j1 < 127 - this.hellRNG.nextInt(5) && j1 > this.hellRNG.nextInt(5))
                    {
                        ...
                        {
                            if (iblockstate2.func_177230_c() == Blocks.netherrack)
                            {
                                if (i1 == -1)
                                {
                                    if (l <= 0)
                                    {
                                        iblockstate = null;
                                        iblockstate1 = Blocks.netherrack.func_176223_P();
                                    }
                                    else if (j1 >= i - 4 && j1 <= i + 1)
                                    {
                                        ...
                                        if (flag)
                                        {
                                            iblockstate = Blocks.soul_sand.func_176223_P();
                                            iblockstate1 = Blocks.soul_sand.func_176223_P();
                                        }
                                    }
                    ...
                    {
                        p_180516_3_.setBlockState(k, j1, j, Blocks.field_150357_h.func_176223_P());
                    }
                }
            }
        }
    ...
    public List<BiomeGenBase.SpawnListEntry> getPossibleCreatures(EnumCreatureType creatureType, BlockPos pos)
    {
        if (creatureType == EnumCreatureType.MONSTER)
        {
            ...
            if (this.genNetherBridge.func_175796_a(this.worldObj, pos) && this.worldObj.func_180495_p(pos.func_177977_b()).func_177230_c() == Blocks.nether_brick)
            {
                return this.genNetherBridge.getSpawnList();
            }
        }
    ...
    {
        return this.provideChunk(blockPosIn.func_177958_n() >> 4, blockPosIn.func_177952_p() >> 4);
    }
}
