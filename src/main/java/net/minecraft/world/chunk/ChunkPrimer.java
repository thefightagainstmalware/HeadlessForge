...
public class ChunkPrimer
{
    ...
    private final IBlockState defaultState = Blocks.field_150350_a.func_176223_P();
    ...
    public IBlockState getBlockState(int index)
    {
        ...
        {
            IBlockState iblockstate = (IBlockState)Block.BLOCK_STATE_IDS.func_148745_a(this.data[index]);
    ...
    public void setBlockState(int index, IBlockState state)
    {
        if (index >= 0 && index < this.data.length)
        {
            this.data[index] = (short)Block.BLOCK_STATE_IDS.func_148747_b(state);
        }
        else
        {
            throw new IndexOutOfBoundsException("The coordinate is out of range");
