...
public class TileEntityBannerRenderer extends TileEntitySpecialRenderer<TileEntityBanner>
{
    ...
    public void renderTileEntityAt(TileEntityBanner te, double x, double y, double z, float partialTicks, int destroyStage)
    {
        ...
        long j = flag ? te.getWorld().func_82737_E() : 0L;
            ...
            if (tileentitybannerrenderer$timedbannertexture == null)
            {
                if (DESIGNS.size() >= 256)
                {
                    ...
                    while (iterator.hasNext())
                    {
                        ...
                        {
                            Minecraft.func_71410_x().func_110434_K().deleteTexture(tileentitybannerrenderer$timedbannertexture1.bannerTexture);
                            iterator.remove();
                        }
                    }
                ...
                Minecraft.func_71410_x().func_110434_K().loadTexture(tileentitybannerrenderer$timedbannertexture.bannerTexture, new LayeredColorMaskTexture(BANNERTEXTURES, list2, list));
