...
public class ContainerHopper extends Container
{
    ...
    public ContainerHopper(InventoryPlayer playerInventory, IInventory hopperInventoryIn, EntityPlayer player)
    {
        ...
        for (int j = 0; j < hopperInventoryIn.func_70302_i_(); ++j)
        {
    ...
    public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
    {
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            if (index < this.hopperInventory.func_70302_i_())
            {
                if (!this.mergeItemStack(itemstack1, this.hopperInventory.func_70302_i_(), this.inventorySlots.size(), true))
                {
                    return null;
                }
            }
            else if (!this.mergeItemStack(itemstack1, 0, this.hopperInventory.func_70302_i_(), false))
            {
                return null;
            }
