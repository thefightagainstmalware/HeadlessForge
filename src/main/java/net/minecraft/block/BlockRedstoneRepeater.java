...
public class BlockRedstoneRepeater extends BlockRedstoneDiode
{
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ)
    {
        ...
        {
            worldIn.func_180501_a(pos, state.cycleProperty(DELAY), 3);
            return true;
        }
    }
    ...
    protected IBlockState getPoweredState(IBlockState unpoweredState)
    {
        ...
        return Blocks.field_150416_aS.func_176223_P().withProperty(FACING, enumfacing).withProperty(DELAY, integer).withProperty(LOCKED, obool);
    }
    ...
    protected IBlockState getUnpoweredState(IBlockState poweredState)
    {
        ...
        return Blocks.field_150413_aR.func_176223_P().withProperty(FACING, enumfacing).withProperty(DELAY, integer).withProperty(LOCKED, obool);
    }
    ...
    public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (this.isRepeaterPowered)
        {
            ...
            double d0 = (double)((float)pos.func_177958_n() + 0.5F) + (double)(rand.nextFloat() - 0.5F) * 0.2D;
            double d1 = (double)((float)pos.func_177956_o() + 0.4F) + (double)(rand.nextFloat() - 0.5F) * 0.2D;
            double d2 = (double)((float)pos.func_177952_p() + 0.5F) + (double)(rand.nextFloat() - 0.5F) * 0.2D;
            float f = -5.0F;
    ...
    {
        return this.func_176223_P().withProperty(FACING, EnumFacing.getHorizontal(meta)).withProperty(LOCKED, Boolean.valueOf(false)).withProperty(DELAY, Integer.valueOf(1 + (meta >> 2)));
    }
