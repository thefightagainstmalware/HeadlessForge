...
public class BlockQuartz extends Block
{
    ...
    public IBlockState onBlockPlaced(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer)
    {
        if (meta == BlockQuartz.EnumType.LINES_Y.getMetadata())
        {
            ...
            {
                case Z:
                    return this.func_176223_P().withProperty(VARIANT, BlockQuartz.EnumType.LINES_Z);

                case X:
                    return this.func_176223_P().withProperty(VARIANT, BlockQuartz.EnumType.LINES_X);

                case Y:
                default:
                    return this.func_176223_P().withProperty(VARIANT, BlockQuartz.EnumType.LINES_Y);
            }
        }
        else
        {
            return meta == BlockQuartz.EnumType.CHISELED.getMetadata() ? this.func_176223_P().withProperty(VARIANT, BlockQuartz.EnumType.CHISELED) : this.func_176223_P().withProperty(VARIANT, BlockQuartz.EnumType.DEFAULT);
        }
    }
    ...
    {
        BlockQuartz.EnumType blockquartz$enumtype = (BlockQuartz.EnumType)state.getValue(VARIANT);
        return blockquartz$enumtype != BlockQuartz.EnumType.LINES_X && blockquartz$enumtype != BlockQuartz.EnumType.LINES_Z ? super.createStackedBlock(state) : new ItemStack(Item.func_150898_a(this), 1, BlockQuartz.EnumType.LINES_Y.getMetadata());
    }
    ...
    {
        return this.func_176223_P().withProperty(VARIANT, BlockQuartz.EnumType.byMetadata(meta));
    }
