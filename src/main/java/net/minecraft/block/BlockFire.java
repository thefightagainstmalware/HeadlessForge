...
public class BlockFire extends Block
{
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos)
    {
        ...
        if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.func_177977_b()) && !Blocks.fire.canCatchFire(worldIn, pos.func_177977_b()))
        {
    ...
    public static void init()
    {
        ...
        Blocks.fire.setFireInfo(Blocks.field_150335_W, 15, 100);
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (worldIn.func_82736_K().func_82766_b("doFireTick"))
        {
            ...
            if (worldIn.provider instanceof WorldProviderEnd && block == Blocks.field_150357_h)
            {
                flag = true;
            }
            ...
            else
            {
                ...
                if (!flag)
                {
                    ...
                    {
                        if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.func_177977_b()) || i > 3)
                        {
                            worldIn.setBlockToAir(pos);
                        }
                    ...
                    if (!this.canCatchFire(worldIn, pos.func_177977_b()) && i == 15 && rand.nextInt(4) == 0)
                    {
                        worldIn.setBlockToAir(pos);
                        return;
                    }
                ...
                this.catchOnFire(worldIn, pos.func_177977_b(), 250 + j, rand, i);
                            ...
                            if (k != 0 || i1 != 0 || l != 0)
                            {
                                ...
                                if (k1 > 0)
                                {
                                    ...
                                    if (l1 > 0 && rand.nextInt(j1) <= l1 && (!worldIn.isRaining() || !this.canDie(worldIn, blockpos)))
                                    {
                                        ...
                                        worldIn.func_180501_a(blockpos, state.withProperty(AGE, Integer.valueOf(i2)), 3);
                                    }
                                }
                            }
                        }
    ...
    {
        int i = this.getFlammability(worldIn.func_180495_p(pos).func_177230_c());
        ...
        if (random.nextInt(chance) < i)
        {
            ...
            if (random.nextInt(age + 10) < 5 && !worldIn.canLightningStrike(pos))
            {
                ...
                worldIn.func_180501_a(pos, this.func_176223_P().withProperty(AGE, Integer.valueOf(j)), 3);
            }
            else
            {
                worldIn.setBlockToAir(pos);
            ...
            {
                Blocks.field_150335_W.onBlockDestroyedByPlayer(worldIn, pos, iblockstate.withProperty(BlockTNT.EXPLODE, Boolean.valueOf(true)));
            }
        }
    }
    ...
    private int getNeighborEncouragement(World worldIn, BlockPos pos)
    {
        ...
        else
        {
            ...
            for (EnumFacing enumfacing : EnumFacing.values())
            {
                i = Math.max(this.getEncouragement(worldIn.func_180495_p(pos.offset(enumfacing)).func_177230_c()), i);
            }
    ...
    {
        return this.getEncouragement(worldIn.func_180495_p(pos).func_177230_c()) > 0;
    }

    public boolean canPlaceBlockAt(World worldIn, BlockPos pos)
    {
        return World.doesBlockHaveSolidTopSurface(worldIn, pos.func_177977_b()) || this.canNeighborCatchFire(worldIn, pos);
    }
    ...
    {
        if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.func_177977_b()) && !this.canNeighborCatchFire(worldIn, pos))
        {
            worldIn.setBlockToAir(pos);
        }
    }
    ...
    public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        ...
        {
            worldIn.playSound((double)((float)pos.func_177958_n() + 0.5F), (double)((float)pos.func_177956_o() + 0.5F), (double)((float)pos.func_177952_p() + 0.5F), "fire.fire", 1.0F + rand.nextFloat(), rand.nextFloat() * 0.7F + 0.3F, false);
        }
        ...
        if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.func_177977_b()) && !Blocks.fire.canCatchFire(worldIn, pos.func_177977_b()))
        {
            if (Blocks.fire.canCatchFire(worldIn, pos.west()))
            {
                for (int j = 0; j < 2; ++j)
                {
                    double d3 = (double)pos.func_177958_n() + rand.nextDouble() * 0.10000000149011612D;
            ...
            if (Blocks.fire.canCatchFire(worldIn, pos.east()))
            {
                for (int k = 0; k < 2; ++k)
                {
                    double d4 = (double)(pos.func_177958_n() + 1) - rand.nextDouble() * 0.10000000149011612D;
            ...
            if (Blocks.fire.canCatchFire(worldIn, pos.north()))
            {
                for (int l = 0; l < 2; ++l)
                {
                    ...
                    double d15 = (double)pos.func_177952_p() + rand.nextDouble() * 0.10000000149011612D;
            ...
            if (Blocks.fire.canCatchFire(worldIn, pos.south()))
            {
                for (int i1 = 0; i1 < 2; ++i1)
                {
                    ...
                    double d16 = (double)(pos.func_177952_p() + 1) - rand.nextDouble() * 0.10000000149011612D;
            ...
            if (Blocks.fire.canCatchFire(worldIn, pos.up()))
            {
                for (int j1 = 0; j1 < 2; ++j1)
                {
                    ...
                    double d12 = (double)(pos.func_177956_o() + 1) - rand.nextDouble() * 0.10000000149011612D;
    ...
    {
        return this.func_176223_P().withProperty(AGE, meta);
    }
