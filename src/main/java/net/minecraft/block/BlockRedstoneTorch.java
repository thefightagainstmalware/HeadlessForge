...
public class BlockRedstoneTorch extends BlockTorch
{
    ...
    private boolean isBurnedOut(World worldIn, BlockPos pos, boolean turnOff)
    {
        ...
        if (turnOff)
        {
            list.add(new BlockRedstoneTorch.Toggle(pos, worldIn.func_82737_E()));
        }
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        ...
        while (list != null && !list.isEmpty() && worldIn.func_82737_E() - ((BlockRedstoneTorch.Toggle)list.get(0)).time > 60L)
        {
            list.remove(0);
        }
        ...
        if (this.isOn)
        {
            if (flag)
            {
                worldIn.func_180501_a(pos, Blocks.unlit_redstone_torch.func_176223_P().withProperty(FACING, state.getValue(FACING)), 3);

                if (this.isBurnedOut(worldIn, pos, true))
                {
                    worldIn.playSoundEffect((double)((float)pos.func_177958_n() + 0.5F), (double)((float)pos.func_177956_o() + 0.5F), (double)((float)pos.func_177952_p() + 0.5F), "random.fizz", 0.5F, 2.6F + (worldIn.rand.nextFloat() - worldIn.rand.nextFloat()) * 0.8F);

                    for (int i = 0; i < 5; ++i)
                    {
                        double d0 = (double)pos.func_177958_n() + rand.nextDouble() * 0.6D + 0.2D;
                        double d1 = (double)pos.func_177956_o() + rand.nextDouble() * 0.6D + 0.2D;
                        double d2 = (double)pos.func_177952_p() + rand.nextDouble() * 0.6D + 0.2D;
                        worldIn.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, d0, d1, d2, 0.0D, 0.0D, 0.0D, new int[0]);
                    }

                    worldIn.scheduleUpdate(pos, worldIn.func_180495_p(pos).func_177230_c(), 160);
                }
            }
        }
        else if (!flag && !this.isBurnedOut(worldIn, pos, false))
        {
            worldIn.func_180501_a(pos, Blocks.redstone_torch.func_176223_P().withProperty(FACING, state.getValue(FACING)), 3);
        }
    }
    ...
    public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (this.isOn)
        {
            double d0 = (double)pos.func_177958_n() + 0.5D + (rand.nextDouble() - 0.5D) * 0.2D;
            double d1 = (double)pos.func_177956_o() + 0.7D + (rand.nextDouble() - 0.5D) * 0.2D;
            double d2 = (double)pos.func_177952_p() + 0.5D + (rand.nextDouble() - 0.5D) * 0.2D;
