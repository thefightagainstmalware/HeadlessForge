...
public class BlockTripWire extends Block
{
    ...
    public void onNeighborBlockChange(World worldIn, BlockPos pos, IBlockState state, Block neighborBlock)
    {
        ...
        boolean flag1 = !World.doesBlockHaveSolidTopSurface(worldIn, pos.func_177977_b());
    ...
    {
        state = state.withProperty(SUSPENDED, Boolean.valueOf(!World.doesBlockHaveSolidTopSurface(worldIn, pos.func_177977_b())));
    ...
    public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player)
    {
        if (!worldIn.isRemote)
        {
            ...
            {
                worldIn.func_180501_a(pos, state.withProperty(DISARMED, Boolean.valueOf(true)), 4);
            }
        }
    }
    ...
    private void notifyHook(World worldIn, BlockPos pos, IBlockState state)
    {
        for (EnumFacing enumfacing : new EnumFacing[] {EnumFacing.SOUTH, EnumFacing.WEST})
        {
            for (int i = 1; i < 42; ++i)
            {
                ...
                if (iblockstate.func_177230_c() != Blocks.tripwire)
                {
                    break;
                }
            }
    ...
    {
        return this.func_176223_P().withProperty(POWERED, Boolean.valueOf((meta & 1) > 0)).withProperty(SUSPENDED, Boolean.valueOf((meta & 2) > 0)).withProperty(ATTACHED, Boolean.valueOf((meta & 4) > 0)).withProperty(DISARMED, Boolean.valueOf((meta & 8) > 0));
    }
