...
public abstract class BlockRedstoneDiode extends BlockDirectional
{
    ...
    {
        return World.doesBlockHaveSolidTopSurface(worldIn, pos.func_177977_b()) ? super.canPlaceBlockAt(worldIn, pos) : false;
    }

    public boolean canBlockStay(World worldIn, BlockPos pos)
    {
        return World.doesBlockHaveSolidTopSurface(worldIn, pos.func_177977_b());
    }
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (!this.isLocked(worldIn, pos, state))
        {
            ...
            else if (!this.isRepeaterPowered)
            {
                ...
                if (!flag)
                {
                    worldIn.updateBlockTick(pos, this.getPoweredState(state).func_177230_c(), this.getTickDelay(state), -1);
                }
            }
        }
    }
    ...
    protected int calculateInputStrength(World worldIn, BlockPos pos, IBlockState state)
    {
        ...
        else
        {
            ...
            return Math.max(i, iblockstate.func_177230_c() == Blocks.redstone_wire ? ((Integer)iblockstate.getValue(BlockRedstoneWire.POWER)).intValue() : 0);
        }
    }
    ...
    {
        return Blocks.field_150413_aR.isAssociated(blockIn) || Blocks.field_150441_bU.isAssociated(blockIn);
    }
    ...
    public boolean isFacingTowardsRepeater(World worldIn, BlockPos pos, IBlockState state)
    {
        ...
        BlockPos blockpos = pos.offset(enumfacing);
        return isRedstoneRepeaterBlockID(worldIn.func_180495_p(blockpos).func_177230_c()) ? worldIn.func_180495_p(blockpos).getValue(FACING) != enumfacing : false;
    }
