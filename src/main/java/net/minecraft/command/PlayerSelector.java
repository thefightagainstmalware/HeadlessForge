...
public class PlayerSelector
{
     ...
     */
    public static EntityPlayerMP func_82386_a(ICommandSender sender, String token)
    {
        return (EntityPlayerMP) func_179652_a(sender, token, EntityPlayerMP.class);
    }

    public static <T extends Entity> T func_179652_a(ICommandSender sender, String token, Class <? extends T > targetClass)
    {
        List<T> list = func_179656_b(sender, token, targetClass);
        return (T)(list.size() == 1 ? (Entity)list.get(0) : null);
    }

    public static IChatComponent func_150869_b(ICommandSender sender, String token)
    {
        List<Entity> list = func_179656_b(sender, token, Entity.class);
    ...
    public static <T extends Entity> List<T> func_179656_b(ICommandSender sender, String token, Class <? extends T > targetClass)
    {
        Matcher matcher = tokenPattern.matcher(token);

        if (matcher.matches() && sender.func_70003_b(1, "@"))
        {
            ...
            else
            {
                ...
                BlockPos blockpos = func_179664_b(map, sender.func_180425_c());
    ...
    private static List<World> getWorlds(ICommandSender sender, Map<String, String> argumentMap)
    {
        ...
        {
            list.add(sender.func_130014_f_());
        }
        else
        {
    ...
    private static <T extends Entity> boolean isEntityTypeValid(ICommandSender commandSender, Map<String, String> params)
    {
        ...
        if (s != null && !EntityList.isStringValidEntityName(s))
        {
            ...
            commandSender.func_145747_a(chatcomponenttranslation);
            return false;
        }
        else
        {
        ...
        if (i != WorldSettings.GameType.NOT_SET.getID())
        {
            list.add(new Predicate<Entity>()
            {
                public boolean apply(Entity p_apply_1_)
                {
                    ...
                    {
                        EntityPlayerMP entityplayermp = (EntityPlayerMP)p_apply_1_;
                        return entityplayermp.field_71134_c.getGameType().getID() == i;
                    }
                }
            });
        }
            ...
            else
            {
                ...
                if (flag && flag2 && !flag1)
                {
                    Predicate<Entity> predicate2 = new Predicate<Entity>()
                    {
                        public boolean apply(Entity p_apply_1_)
                        {
                            return p_apply_1_.field_70165_t >= axisalignedbb.minX && p_apply_1_.field_70163_u >= axisalignedbb.minY && p_apply_1_.field_70161_v >= axisalignedbb.minZ ? p_apply_1_.field_70165_t < axisalignedbb.maxX && p_apply_1_.field_70163_u < axisalignedbb.maxY && p_apply_1_.field_70161_v < axisalignedbb.maxZ : false;
                        }
                    };
    ...
    {
        return new BlockPos(parseIntWithDefault(p_179664_0_, "x", p_179664_1_.func_177958_n()), parseIntWithDefault(p_179664_0_, "y", p_179664_1_.func_177956_o()), parseIntWithDefault(p_179664_0_, "z", p_179664_1_.func_177952_p()));
    }
